...
def main() -> None:
    ...
    agent_user = Agent(
        "openai:gpt-4o",
        history_processors=[user_message_filter]
    )
    result_1 = agent_user.run_sync(
        "Please summarize the whole chat history until now.",
    ...
    agent_model = Agent(
        "openai:gpt-4o",
        history_processors=[model_message_filter]
    )
    try:
        result_2 = agent_model.run_sync(
            "Please summarize the whole chat history until now.",
            message_history=history
        )
